...
export class BeSoccerAdapter extends BaseAdapter {
  
  public async getPlayerData(uuid: PlayerUUID): Promise<any> {
    // BeSoccer uses player IDs in URLs like: https://www.besoccer.com/player/mason-greenwood/351549
  ...
  public async parseHtml(html: string, url: string, uuid: PlayerUUID, $?: ReturnType<typeof cheerio.load>, playerId?: string): Promise<any> {
    ...
    // ── 5. Boy ─────────────────────────────────────────────────────────────
    const height = $('.player-height').text().trim() ||
                  $('.header-height').text().trim() ||
                  '';

    // ── 6. Sezon İstatistikleri ─────────────────────────────────────────────
    let seasonStats: any = {};
